...
@RelationBuilder
public class Rule4AntipatternBuilder extends AntipatternFinderInIfStatements implements IRelationBuilder<MIfStatement, MCompilationUnit>{
	...
	public Group<MIfStatement> buildGroup(MCompilationUnit arg0) {
		...
		return collectRule4Antipatterns(isPresentInvocationGroup);
	}

	private Group<MIfStatement> collectRule4Antipatterns(Group<MInvocation> isPresentGroup) {
		...
		.filter(ifStatement -> isAntipattern(Antipattern.RULE_4_ANTIPATTERN, ifStatement, invocatorName.getValue0().get()))
